    ...
    async def prepare_messages(self, message: str, attachments: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for attachment in attachments:
            if attachment.get("mime_type", "").startswith("image/"):
                content.append({"type": "image", "image": attachment.get("data", "")})
...
class GeminiProvider(AIProvider):
    ...
    async def prepare_messages(self, message: str, attachments: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [{"role": "user", "parts": [{"text": message}] + [{"inline_data": item} for item in attachments]}]
...
class QwenProvider(AIProvider):
    ...
    async def prepare_messages(self, message: str, attachments: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [{"role": "user", "content": [{"type": "text", "text": message}] + [{"type": "image", "image": item.get("data", "")} for item in attachments]}]
...
class AnthropicProvider(AIProvider):
    ...
    async def prepare_messages(self, message: str, attachments: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [{"role": "user", "content": [{"type": "text", "text": message}] + [{"type": "image", "source": item} for item in attachments]}]
